...
class CustomToneListWindow(Adw.Window):
    ...
    def __init__(self, parent, db, eds, mode="sms"):
        ...
        try:
            ...
            if self.eds:
                sig_id = self.eds.connect('contact-modified', lambda *args: GLib.idle_add(self._refresh_list))
                self.eds_signals.append((self.eds, sig_id))
                sig_id = self.eds.connect('contact-added', lambda *args: GLib.idle_add(self._refresh_list))
                self.eds_signals.append((self.eds, sig_id))
